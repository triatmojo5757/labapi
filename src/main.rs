...
async fn main() -> anyhow::Result<()> {
    ...
    let digiflazz_username = std::env::var("DIGIFLAZZ_USERNAME")?.trim().to_string();
    let digiflazz_dev_key = std::env::var("DIGIFLAZZ_DEV_KEY")?.trim().to_string();
    let digiflazz_prod_key = std::env::var("DIGIFLAZZ_PROD_KEY")?.trim().to_string();
    let digiflazz_mode = std::env::var("DIGIFLAZZ_MODE").unwrap_or_else(|_| "dev".to_string());
    let digiflazz_use_production =
        matches!(digiflazz_mode.as_str(), "prod" | "production" | "live");
    ...
    let public = Router::new()
        ...
        .route(
            "/journals/public",
            get(routes::journals::list_journals_public),
        )
        .route(
            "/journals/list_all",
    ...
    let protected = Router::new()
        ...
        .route(
            "/accounts/:account_id/pin",
            patch(routes::accounts::update_account_pin),
        )
        .route(
            "/journals",
            post(routes::journals::post_journal).get(routes::journals::list_journals),
        )
        ...
        .route(
            "/accounts/update_widhraw_journal",
            post(routes::cash::update_widhraw_journal),
        )
        .route("/accounts/check_pin", post(routes::accounts::check_pin))
        .route(
            "/accounts/list_rekening_pt",
            get(routes::accounts::list_rekening_pt),
        )
        .route(
            "/profile/fcm-token",
            patch(routes::profile::update_fcm_token),
        )
        .route(
            "/accounts/get_rekening_by_no_account",
            get(routes::accounts::get_rekening_by_no_account),
        )
        .route(
            "/digiflazz/products",
        ...
        .route(
            ...
            post(routes::digiflaz::inquiry_pln),
        )
        .route(
            "/digiflazz/inq-pasca",
            post(routes::digiflaz::inquiry_pasca_digiflazz),
        )
        .route(
            "/digiflazz/status-pasca/:ref_id",
            get(routes::digiflaz::status_pasca_digiflazz),
        )
        .route(
            "/digiflazz/pay-pasca",
            post(routes::digiflaz::pay_pasca_digiflazz),
        )
        .route("/digiflazz/topup", post(routes::digiflaz::topup_digiflazz))
        .route(
            "/digiflazz/cek-status/:ref_id",
            get(routes::digiflaz::cek_status_digiflazz),
        )
        .route(
        ...
        .route(
            "/investment/insert_saham_tf",
            post(routes::investment::insert_saham_tf),
        )
        .route(
            "/investment/invest_today",
            get(routes::investment::get_invest_today),
        )
        .route(
            "/investment/deviden_detail",
            get(routes::investment::get_deviden_detail),
        )
        .route(
            "/investment/dashboard_deviden",
